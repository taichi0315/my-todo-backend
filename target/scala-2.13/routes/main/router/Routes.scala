...
class Routes(
  ...
  def documentation = List(
    ("""GET""", this.prefix + (if(this.prefix.endsWith("/")) "" else "/") + """todos""", """controllers.TodoController.get(state:Option[String])"""),
    Nil
  ...
  private[this] lazy val controllers_TodoController_get0_invoker = createInvoker(
    TodoController_0.get(fakeValue[Option[String]]),
  ...
  def routes: PartialFunction[RequestHeader, Handler] = {
    ...
    case controllers_TodoController_get0_route(params@_) =>
      call(params.fromQuery[Option[String]]("state", None)) { (state) =>
